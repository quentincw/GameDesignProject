  ...
  while( running )
  {
    ...
    while( SDL_PollEvent( &e ) != 0 )
    {
      ...
      if( e.type == SDL_KEYDOWN )
      {
        ...
        if( e.key.keysym.sym == SDLK_r ) SDL_FillRect( screenSurface, NULL, SDL_MapRGB( screenSurface->format, 255, 0, 0 ) );
        if( e.key.keysym.sym == SDLK_g ) SDL_FillRect( screenSurface, NULL, SDL_MapRGB( screenSurface->format, 0, 255, 0 ) );
        if( e.key.keysym.sym == SDLK_b ) SDL_FillRect( screenSurface, NULL, SDL_MapRGB( screenSurface->format, 0, 0, 255 ) );
      }
